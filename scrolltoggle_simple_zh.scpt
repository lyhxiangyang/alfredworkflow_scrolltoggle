on run
	...
	tell application "System Settings"
		...
		repeat until paneLoaded or attemptCount > 20
			try
				-- 尝试设置当前面板为 触控板
				set current pane to pane "触控板"
				
				-- 检查是否成功设置
				if current pane is pane "触控板" then
	...
	tell application "System Events"
		try
			...
			tell process "System Settings"
				...
				set attemptCount to 0 -- 初始化计数器
				-- 循环等待 "自然滚动" 复选框加载
				repeat until (exists (checkbox "自然滚动" of group 1 of scroll area 1 of group 1 of group 2 of splitter group 1 of group 1 of trackpadWindow)) or attemptCount > 20
					set attemptCount to attemptCount + 1
					delay 0.1
				end repeat
				...
				if attemptCount > 20 then
					tell application "System Settings" to quit
					log "无法加载 自然滚动 复选框"
					return "操作失败，无法加载 自然滚动 复选框"
				end if
				
				-- 点击 "自然滚动" 的复选框
				set targetCheckbox to checkbox "自然滚动" of group 1 of scroll area 1 of group 1 of group 2 of splitter group 1 of group 1 of trackpadWindow
